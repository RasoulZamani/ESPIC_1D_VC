...
def emiter_step (
        omega_p,    # plasma freq of beam for getting density
        Ne,         # number of super particles emits in each time step
        dx,         # space step (grid size)
        v0,         # initial velocity of particles
        ):
    
    """ emites particles for one step in times

    Returns:
        emits (list): array of Ne particles with v0 and Wp
    """
    class EParticle():
        def __init__(self,
                 pos, # position of particles
            ...
            self.x = pos
            self.v = vel
            # from wp^2 = n*q^2 / m*eps  and n=Ne/dx we drecive q:
            self.q = omega_p**2 * (1 / QoverM) * EPS0 * (dx/Ne)
            self.qm = QoverM
            self.mv = move
    ...
    emits = []
    for i in range(Ne):
        new_particle = EParticle(pos = float(np.random.uniform(0,dx,1)),
                            vel = v0, omega_p = omega_p,
                            QoverM = QM_e, move=True)
        emits.append(new_particle)
